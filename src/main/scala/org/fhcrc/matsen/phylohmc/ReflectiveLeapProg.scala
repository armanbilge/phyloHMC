...
trait ReflectiveLeapProg[R, N] extends PhyloHMC[R, N] {
  ...
  def leapprog(eps: R)(z: Z[R, N]): Z[R, N] = {
    ...
    qp.lengths.indices.view.map(solveForEps(z)).zipWithIndex.filter(_._1.isDefined).map(ei => (ei._1.get, ei._2)).filter(_._1 <= eps).sortWith(_._1 < _._1).view.map(_._2).foldLeft(zp.copy(q = qp)(_U = U(qp))) { (z, i) =>
